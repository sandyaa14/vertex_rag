...
# Step 5: Upload extracted text into RAG corpus (✅ Final Fix)
async def upload_text_to_rag(corpus):
    """Upload extracted text from GCS into the RAG corpus."""
    response = await rag.import_files_async(
        corpus=corpus.name,  # ✅ Corrected parameter
        gcs_source_uris=[GCS_URI]  # ✅ Ensure correct argument name & use list
    )
