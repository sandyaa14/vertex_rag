...
# Step 5: Upload the extracted text into RAG corpus
async def upload_text_to_rag(corpus):
    """Upload extracted text from GCS into the RAG corpus."""
    response = await rag.upload_file(
        corpus_name=corpus.name,
        gcs_source_uri=GCS_URI  # 🔹 Fix: Corrected the parameter name
    )
...
async def run_rag_query(corpus):
    ...
    response = await rag.rag_retrieval(
        corpus_name=corpus.name,  # 🔹 Fix: Ensure corpus name is passed correctly
        query=query
    )
