...
# ✅ Correct way to use retrieval
retrieval_source = rag.VertexRagStore(
    rag_resources=[rag.RagResource(rag_corpus=CORPUS_ID)]
)

# ✅ Initialize GenerativeModel with retrieval set up separately
...
# ✅ Fetch retrieved documents separately using `retrieve_and_generate_content()`
response = model.retrieve_and_generate_content(
    [Part.from_text(query)],
    generation_config={"temperature": 0.2},
    retrieval_config=rag.RagRetrievalConfig(vertex_rag=retrieval_source)  # ✅ Corrected retrieval usage
)
